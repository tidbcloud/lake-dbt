...
MACROS__VALIDATE__LAKE__LIST_RELATIONS_WITHOUT_CACHING = """
{% macro validate_list_relations_without_caching(schema_relation) %}
    {% set relation_list_result = tidbcloudlake__list_relations_without_caching(schema_relation, max_iter=11, max_results_per_iter=10) %}
    {% set n_relations = relation_list_result | length %}
    {{ log("n_relations: " ~ n_relations) }}
{% endmacro %}
"""

MACROS__VALIDATE__LAKE__LIST_RELATIONS_WITHOUT_CACHING_RAISE_ERROR = """
{% macro validate_list_relations_without_caching_raise_error(schema_relation) %}
    {{ tidbcloudlake__list_relations_without_caching(schema_relation, max_iter=33, max_results_per_iter=3) }}
{% endmacro %}
"""
...
class TestListRelationsWithoutCachingSingle:
    ...
    def macros(self):
        return {
            "validate_list_relations_without_caching.sql": MACROS__VALIDATE__LAKE__LIST_RELATIONS_WITHOUT_CACHING,
        }

    def test__tidbcloudlake__list_relations_without_caching_termination(self, project):
        """
        validates that we do NOT trigger pagination logic tidbcloudlake__list_relations_without_caching
