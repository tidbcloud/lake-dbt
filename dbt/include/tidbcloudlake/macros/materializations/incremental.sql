{% materialization incremental, adapter='tidbcloudlake' %}
  ...
  {% elif unique_key is none -%}
    {% call statement('main') %}
        {{ tidbcloudlake__insert_into(target_relation, sql) }}
    {% endcall %}
  ...
  {% else %}
    ...
    {% if incremental_strategy == 'delete_insert' %}
      {% do tidbcloudlake__incremental_delete_insert(existing_relation, unique_key, incremental_predicates) %}
...
{% macro tidbcloudlake__incremental_delete_insert(existing_relation, unique_key, incremental_predicates) %}
