...
{% macro tidbcloudlake__create_schema(relation) -%}
...
{% macro tidbcloudlake__drop_relation(relation) -%}
...
{% macro tidbcloudlake__drop_schema(relation) -%}
...
{% macro tidbcloudlake__get_columns_in_relation(relation) -%}
...
{% macro tidbcloudlake__list_relations_without_caching(schema_relation) -%}
...
{% macro tidbcloudlake__list_schemas(database) -%}
...
{% macro tidbcloudlake__rename_relation(from_relation, to_relation) -%}
...
{% macro tidbcloudlake__truncate_relation(relation) -%}
...
{% macro tidbcloudlake__make_temp_relation(base_relation, suffix) %}
  {# Keep the schema: the session has no current database, so temporary
     relations have to be schema-qualified like every other relation. #}
...
{% macro tidbcloudlake__generate_database_name(custom_database_name=none, node=none) -%}
  {% do return(None) %}
{%- endmacro %}

{% macro tidbcloudlake__current_timestamp() -%}
  NOW()
{%- endmacro %}

{% macro tidbcloudlake__create_table_as(temporary, relation, sql) -%}
  {%- set sql_header = config.get('sql_header', none) -%}

  {{ sql_header if sql_header is not none }}

  {% if temporary -%}
    create or replace transient table {{ relation.include(database=False) }}
  {%- else %}
...
{% macro tidbcloudlake__create_view_as(relation, sql) -%}
  {%- set sql_header = config.get('sql_header', none) -%}
...
{% macro tidbcloudlake__get_columns_in_query(select_sql) %}
