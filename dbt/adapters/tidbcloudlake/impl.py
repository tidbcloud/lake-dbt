...
import csv
import io
from dbt.adapters.tidbcloudlake.column import TiDBCloudLakeColumn
from dbt.adapters.tidbcloudlake.connections import TiDBCloudLakeConnectionManager
...
@dataclass
class TiDBCloudLakeConfig(AdapterConfig):
    cluster_by: Optional[Union[List[str], str]] = None


class TiDBCloudLakeAdapter(SQLAdapter):
    Relation = TiDBCloudLakeRelation
    Column = TiDBCloudLakeColumn
    ConnectionManager = TiDBCloudLakeConnectionManager
    AdapterSpecificConfigs = TiDBCloudLakeConfig
    ...
    def list_relations_without_caching(
            self, schema_relation: TiDBCloudLakeRelation
    ) -> List[TiDBCloudLakeRelation]:
    ...
    def _catalog_filter_table(
        ...
        # This adapter treats database and schema as the same thing and always
        # reports database as None, so the base implementation -- which drops
        # every row whose (database, schema) pair has a None database -- would
        # filter the whole catalog away. Match on schema alone instead.
        ...
        schemas = frozenset(
            schema.lower() for _, schema in used_schemas if schema is not None
        )
    ...
    def parse_show_columns(
            self, _relation: TiDBCloudLakeRelation, raw_rows: List[agate.Row]
    ) -> List[TiDBCloudLakeColumn]:
        rows = [
    ...
    def get_columns_in_relation(
            self, relation: TiDBCloudLakeRelation
    ) -> List[TiDBCloudLakeColumn]:
    ...
    def _get_one_catalog(
        ...
        if len(schemas) != 1:
            DbtRuntimeError(
                f"Expected only one schema in TiDB Cloud Lake _get_one_catalog, found {schemas}"
            )
    ...
    def get_rows_different_sql(
            self,
            relation_a: TiDBCloudLakeRelation,
            relation_b: TiDBCloudLakeRelation,
